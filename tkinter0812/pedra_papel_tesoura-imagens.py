...
app_pessoa_linha = Label(
    frame_cima, text="", bg=cor4, relief="flat"
)
app_pessoa_linha.place(x=0, y=0, width=4, height=140)

app_pc_linha = Label(
    frame_cima, text="", bg=cor4, relief="flat"
)
app_pc_linha.place(x=396, y=0, width=4, height=140)

# --- CONFIGURANDO A BARRA HORIZONTAL (SEMPRE AMARELA) ---
app_linha_horizontal = Label(
    frame_cima, text="", bg=cor2, relief="flat"
)
app_linha_horizontal.place(x=0, y=140, width=400, height=10)
...
def jogar(escolha_usuario):
    ...
    if escolha_usuario == escolha_pc:
        ...
        or
        (escolha_usuario == "Papel" and escolha_pc == "Pedra")
        or
    ...
    # Altera somente as cores das barras laterais
    app_pessoa_linha.config(bg=cor_lateral)
    app_pc_linha.config(bg=cor_lateral)

    # A barra horizontal permanece sempre amarela
    app_linha_horizontal.config(bg=cor2)
